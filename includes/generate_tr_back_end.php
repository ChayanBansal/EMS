<?php
    ...
    if(isset($_POST['tab_submit']))
    {
        ...
        $get_ac_session_id_run=mysqli_run($conn,$get_ac_session_id);
        if(mysqli_num_rows($get_ac_session_id_run)>0)
        {
            ...
            while($roll_id=mysqli_fetch_assoc($get_roll_id_run)) //$roll_id['roll_id'] $roll_id['enrol_no']
            {
                ...
                $get_sub_id="SELECT sub_id, practical_flag, credits_allotted FROM sub_distribution WHERE sub_code IN 
                            (
                                (SELECT sub_code FROM subjects WHERE ac_session_id=$ac_session_id AND ((elective_flag=0) OR (elective_flag=1 AND sub_code IN 
