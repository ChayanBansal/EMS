...
if (isset($_POST['proceed_to_feed'])) {
    ...
    switch ($examType) {
        ...
        case 'retotal':
            ...
            $ac_sess_id = mysqli_fetch_assoc($ac_sess_id)['ac_session_id'];
            $_SESSION['ac_sess_id'] = $ac_sess_id;
            $retotal_sess_id = "SELECT retotal_session_id FROM retotal_sessions WHERE ac_session_id=$ac_sess_id";
            $retotal_sess_id = mysqli_query($conn, $retotal_sess_id);
            $retotal_sess_id = mysqli_fetch_assoc($retotal_sess_id)['retotal_session_id'];
            $_SESSION['retotal_session_id'] = $retotal_sess_id;
        /* $check_exists_audit_qry = "SELECT count(*) as 'count' from auditing where session_id=$ac_sess_id AND component_id=" . $_SESSION['sub_comp_id'] . " AND ac_sub_code IN(SELECT ac_sub_code FROM subjects WHERE sub_code='" . $_SESSION['sub_code'] . "' AND ac_session_id=$ac_sess_id)";
        $check_exists_audit_qry_run = mysqli_query($conn, $check_exists_audit_qry);
        if ($check_exists_audit_qry_run) {
            $count = mysqli_fetch_assoc($check_exists_audit_qry_run);
...
?>
 ...
 <form action="" method="post">
    ...
    $elective_flag = mysqli_fetch_assoc($get_elective_flag_run);
    if ($elective_flag['elective_flag'] == 0) {
        $get_stud_qry = "SELECT r.enrol_no,first_name,last_name,father_name,r.roll_id from students s,roll_list r,atkt_roll_list atktrl where atktrl.atkt_roll_id IN(SELECT atkt_roll_id FROM atkt_subjects WHERE sub_id=".$_SESSION['sub_id']." AND component_id=".$_SESSION['sub_comp_id'] .") AND atktrl.roll_id=r.roll_id AND atktrl.atkt_session_id IN(SELECT atkt_session_id FROM atkt_sessions WHERE ac_session_id=" . $_SESSION['ac_sess_id'] . ") AND s.enrol_no=r.enrol_no AND r.semester=" . $_SESSION['semester'] . " AND r.atkt_reg_flag=1";
        $get_stud_qry_run = mysqli_query($conn, $get_stud_qry);
        if ($get_stud_qry_run) {
            $row_count = 1;
            while ($row = mysqli_fetch_assoc($get_stud_qry_run)) {
            ...
            $alert = new alert();
            $alert->exec("No students found! Make sure students are registered for ATKT.", "info");
        }
    } else {
...
$validate->conf_logged_in();
$options = new useroptions(); //inserting marks
$options->insert_retotal_marks($conn);
$obj = new head();
$obj->displayheader();
require('../preloader/preload.php');
$obj->dispmenu(4, ["/ems/includes/home", "/ems/includes/logout", "/ems/includes/useroptions", "/ems/includes/developers"], ["glyphicon glyphicon-home", "glyphicon glyphicon-log-out", 'glyphicon glyphicon-th', "glyphicon glyphicon-info-sign"], ["Home", "Log Out", "Options", "About Us"]);
...
?>
 ...
 <form action="" method="post">
    ...
    $elective_flag = mysqli_fetch_assoc($get_elective_flag_run);
    if ($elective_flag['elective_flag'] == 0) {
        $get_stud_qry = "SELECT r.enrol_no,first_name,last_name,father_name,r.roll_id from students s,roll_list r,retotal_roll_list retotalrl where retotalrl.roll_id=r.roll_id AND retotalrl.retotal_roll_id IN(SELECT retotal_roll_id FROM retotal_subjects WHERE sub_id=".$_SESSION['sub_id'].") AND retotalrl.retotal_session_id IN(SELECT retotal_session_id FROM retotal_sessions WHERE ac_session_id=" . $_SESSION['ac_sess_id'] . ") AND s.enrol_no=r.enrol_no AND r.semester=" . $_SESSION['semester'] . " AND r.retotal_reg_flag=1";
        $get_stud_qry_run = mysqli_query($conn, $get_stud_qry);
        if ($get_stud_qry_run) {
            $row_count = 1;
            while ($row = mysqli_fetch_assoc($get_stud_qry_run)) {
            ...
            $alert = new alert();
            $alert->exec("No students found! Make sure students are registered for Retotal.", "info");
        }
    } else {
  ...
  <div class="remarks">
      ...
      <?php
        ...
        ?>
      <span id="controls"><center><?php
                                    $btn->display_btn("", "btn btn-primary", "submit", "feed_retotal_marks", "", "Submit All"); ?></span>
      </center></div>
